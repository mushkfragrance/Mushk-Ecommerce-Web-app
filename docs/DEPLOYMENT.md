...
GitHub is the deploy source: `https://github.com/mushkfragrance/Mushk-Ecommerce-Web-app`. Commit and push `main` before importing (local `.env` stays gitignored).
...
When you attach `www` / `admin` / `api.mushkfragrance.com`, update Railway CORS, Vercel `VITE_*` vars, and rebuild. See Hostinger DNS below.
